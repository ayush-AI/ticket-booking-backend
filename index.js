...
app.post("/book", (req, res) => {
  ...
  if (!numberOfSeats || numberOfSeats < 1 || numberOfSeats > 7) {
    return res
      .status(400)
      .json({
        error: "Invalid number of seats requested. Must be between 1 and 7.",
      });
  }
...
app.delete("/cancel/:bookingId", (req, res) => {
  ...
  const canceledSeats = seats.filter(
    (seat) => seat.booking_id === parseInt(bookingId)
  );
